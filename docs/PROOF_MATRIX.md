...
| PM-10 | Meta-transition requires refinement proof | `TransitionRefinement` defines witnessed forward-simulation `F_old ⊑ F_new`; K0 gate remains separate | NEAR-COMPLETE (~98% contract level) | T24–T29 | Connect refinement witness to MetaTransition and define closure/fixed-point obligations |
