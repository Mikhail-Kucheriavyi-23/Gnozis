...
| PM-10 | Meta-transition requires refinement proof | `TransitionRefinement` + `ClosureObligation` + `FixedPointObligation`; K0 gate remains separate | NEAR-COMPLETE (~99% contract level) | T24–T29 | Bind all obligations into one meta-transition admissibility contract and distinguish witness evidence from universal proof |
