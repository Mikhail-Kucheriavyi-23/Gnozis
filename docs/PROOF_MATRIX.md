...
| PM-10 | Meta-transition requires refinement proof | `RefinementProof` now supports executable refinement predicate + K0 preservation; `MetaTransition` is fail-closed | NEAR-COMPLETE (~95% contract level) | T24–T29 | Define canonical F→F' refinement relation plus closure/fixed-point obligations |
